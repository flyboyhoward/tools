...
def load_gt_bbox(filepath):
    with open(filepath) as f:
        data = f.read()
    objs = re.findall('\d+,\d+,\d+,\d+,\d+,\d+,\d+,\d+', data)

    objs_str = str(objs)
    objs_list = objs_str.replace(',',' ')
    objs = re.findall('\d+ \d+ \d+ \d+ \d+ \d+ \d+ \d+',objs_list)
...
def main():
    ...
    args.images_dir = '/home/wwh/Desktop/VisDrone2018-VID-val/Images'
    args.annots_dir = '/home/wwh/Desktop/VisDrone2018-VID-val/Annotations'
    args.output_dir = '/home/wwh/Desktop/VisDrone2018-VID-val/output'
    ...
    for image_filename in os.listdir(args.images_dir):
        ...
        for bb in gtBBs:
            draw(bb, im_copy, color=[255,0,0]) # color: green
